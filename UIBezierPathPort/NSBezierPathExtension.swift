#if os(OSX)
  import Cocoa
  struct NSRectCorner : RawOptionSetType {
  ...
  //This doesn't work as of Beta 5
  //typelias UIBezierPath = NSBezierPath
  ...
  class UIBezierPath : NSBezierPath, NSBezierPathExtension {
    ...
    convenience required init(rect: NSRect) {
      self.init(roundedRect:rect, cornerRadius:0)
    }
    //
    convenience required init(ovalInRect: NSRect) {
      self.init()
    ...
    convenience required init(roundedRect rect: NSRect, cornerRadius: CGFloat) {
      
      self.init(roundedRect:rect, byRoundingCorners:.AllCorners, cornerRadii:NSSize(width: cornerRadius, height: cornerRadius))
    }
    
    
    convenience required init(roundedRect rect: NSRect, byRoundingCorners corners: UIRectCorner, cornerRadii: NSSize) {
      var path = CGPathCreateMutable();
    ...
    convenience required init(arcCenter center: CGPoint, radius: CGFloat, startAngle: CGFloat, endAngle: CGFloat, clockwise: Bool) {
      self.init()
    ...
    convenience required init(CGPath: CGPathRef!) {
      self.init()
      self.CGPath = CGPath
    }
